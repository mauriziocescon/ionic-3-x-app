...
export class TodoDataService {
  ...
  public mergeSectionsTasksDb(newSecsDb: Array<SectionDb>): any {
    return new Promise((resolve, reject) => {
      this.storage.ready()
        .then(() => {
          this.storage.get(this.appConstants.Application.TODO_DB_KEY)
            .then((secsDb: Array<SectionDb>) => {
              ...
              for (let newSecDb of newSecsDb) {
                ...
                if (newSecDbIndex === -1) {
                  ...
                  let secDb = secsDb[newSecDbIndex];

                  // update
                  secDb.creationDate = newSecDb.creationDate;
                  secDb.type = newSecDb.type;
                  secDb.description = newSecDb.description;

                  let tasksDbIds = secDb.tasks.map((taskDb) => {
                    return taskDb.id
                  });
                  ...
                  for (let newTaskDb of newSecDb.tasks) {
                    ...
                    if (newTaskDbIndex === -1) {
                      ...
                      let taskDb = secDb.tasks[newTaskDbIndex];

                      // update
                      taskDb.creationDate = newTaskDb.creationDate;
                      taskDb.description = newTaskDb.description;
                      taskDb.done = newTaskDb.done;
                    }
                  }
                }
              }
